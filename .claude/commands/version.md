...
It is a command rather than an agent for the same reason `/release` is: the commit, push, and `gh pr create` steps are deliberately left off the allowlist in `.claude/settings.json` so they surface for human approval, which only happens in the main session. See issue #62.
